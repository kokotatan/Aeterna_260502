...
async function fetchRelevantNews(keywords) {
  ...
  return [
    ...
    '・海面上昇により沿岸都市が水没リスク増大、国連が緊急警告',
    '・気候変動による農業被害、世界食料安保に深刻な影響',
  ].join('\n');
}
...
async function runJudgment() {
  ...
  if (persona.status !== 'active') {
    return { triggered: false, reason: 'Persona is not active.' };
  }
  ...
  if (persona.last_triggered) {
    ...
    if (minutesSince < (persona.cooldown_minutes || 60)) {
      return {
        triggered: false,
        reason: `Cooldown active (${Math.round(minutesSince)}min / ${persona.cooldown_minutes}min).`,
      };
    }
  }
...
あなたのタスク:
- 提供されたニュース情報を分析する
- トリガーキーワードに関連する重大なイベントが発生しているか判断する
- トリガーされた場合、どの受取人に、いくら送金するかを決定する
- 必ず有効なJSONのみを返す（説明文不要）

トリガーキーワード: ${persona.trigger_keywords.join(', ')}

受取人リスト:
${persona.recipients.map(r => `- ${r.name} (wallet: ${r.wallet}, ratio: ${r.ratio}, cause: ${r.cause})`).join('\n')}

総XRP予算: ${persona.total_xrp} XRP`;

  const userMessage = `本日のニュース:\n${newsText}\n\n上記ニュースを分析し、以下のJSON形式で回答してください:
{
  "triggered": true/false,
  "recipients": [
    {"name": "受取人名", "wallet": "ウォレットアドレス", "amount_xrp": 送金額}
  ],
  "reason": "故人の言葉で書いた判断理由（日本語、100文字程度）",
  "keywords_found": ["検出されたキーワード"]
}
triggered=falseの場合は recipients は空配列で、reasonには送金しない理由を記述してください。`;
