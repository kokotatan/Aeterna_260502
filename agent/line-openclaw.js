...
- 「家族コード発行」→ generate_family_code を呼ぶ → コードを返す
- 「始める / コマンド / ヘルプ / help」→ 使えるコマンド一覧を表示（デジタルツイン返答しない）
...
async function executeDonationCycle(userId, user, force, { updateUserFn, pushFn }) {
  ...
  // ── Step 1: Google Search でリアルタイムニュース取得 ─────────────────
  const keywords = [
    ...(persona.trigger_keywords || []),
    ...(persona.values || []),
  ].filter(Boolean).slice(0, 5).join(' ');

  const searchQuery =
    `以下のキーワードに関連する最新ニュース（2025〜2026年）を3〜5件、日本語で箇条書きにしてください。\n` +
    `キーワード: ${keywords || persona.trigger_condition}\n` +
    `形式: 「・[見出し]: [1〜2文の概要]」`;
  ...
  let newsText = '';
  try {
    const searchRes = await searchModel.generateContent(searchQuery);
    newsText = searchRes.response.text().trim();
    console.log(`[DONATION] News fetched (${newsText.length} chars)`);
  } catch (err) {
    console.warn('[DONATION] Search failed, using fallback:', err.message);
    newsText = `・${persona.trigger_condition} に関する動向が世界的に注目されています`;
  }

  // ── Step 2: ニュースとペルソナを照合してJSON判断 ──────────────────
  const judgePrompt =
    `あなたは「${persona.name}」の分身AIエージェントです。\n\n` +
    `${bioBlock}` +
    ...
    `【本日のニュース】\n${newsText}\n\n` +
    `上記ニュースはトリガー条件に合致しますか？判断し、\n` +
    `以下パートナー候補から「${persona.name}」の価値観・トリガー条件に最も合う1〜2件を選んでください:\n` +
    `${partnerJson}\n\n` +
    `letterは ${persona.name} の語り口・口癖で書いた2〜3文の手紙（世界の動きと価値観・トリガーを結びつける）。\n` +
    `whyは選んだ団体がこの人のトリガー条件・価値観に合う具体的な理由（40文字）。\n\n` +
    `JSON形式のみで返してください:\n` +
    `{"triggered":true,"news":["ニュース1","ニュース2","ニュース3"],"letter":"手紙2〜3文",` +
    `"selected":[{"name":"団体名（候補リストから正確に）","category":"カテゴリ","ratio":0.6,"why":"理由40文字"}]}`;

  const judgeModel = genAI.getGenerativeModel({ model: 'gemini-2.0-flash' });
  const res = await judgeModel.generateContent(judgePrompt);
